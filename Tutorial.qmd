...
This true score variance $\sigma^2_T$ describes the variation in the scores, that can be attributed to genuine differences in the underlying factor being measured. Error score variance $\sigma^2_E$ describes the variation in the scores, that can be attributed to random, unsystematic variation. It quantifies the random measurement error that is introduced by the instruments inability to perfectly measure the underlying factor. Reliability coefficients thereby express the proportion of observed score variance that cannot be attributed to random measurement error, and takes on values between 0 and 1 [@lord1974].
...
**When is this the right tool?** This approach is well suited when the research question concerns the *signal-to-noise ratio* of a scale across populations that are expected to differ in the spread of the underlying construct. Consider an ability test developed and validated in a sample of university students. This describes a common practice driven by convenience, that persists despite growing recognition that student samples are not representative of the general population [@petrutiu2026]. University students are a relatively homogeneous group with respect to ability. Students likely show less variation in ability than the general population. However, if a researchers wishes to apply the ability test to the sample of the general population in order to differentiate between that population's ability levels, the absolute extent of random measurement error is not of interest.

In this context, the relative amount of measurement error, the signal-to-noise ratio as assessed through score reliability is of central interest in terms of measurement quality. If score reliability for the general population samples is approximately equal to score reliability for the university student sample, then the ability test discriminates equally well between the levels of ability in both groups. If it differs, the reasons for those differences and their practical implications warrant further investigation [e.g. @gnambs2015]. Reliability Generalization Meta-Analysis can reveal this directly, informing decisions about whether the test usefully differentiates among individuals within both contexts.
...
Consider the following for our ability test example. Suppose we observe a Cronbach's alpha of .67 both for the student sample and the general population sample.

$$\rho_{xx'\ students} = \frac{2}{2 + 1} = .67 \qquad \rho_{xx'\ general} = \frac{3}{3 + 1.5} = .67$$ {#eq-RGMA2}

In this pattern, we observe identical score reliability and differences in true and error score variance. True and error score variance retain the same proportion in both samples. This means that the relative amount of measurement error in the observed scores is identical across both samples. This is the core mechanism, why the test can be used to differentiate equally well between different ability levels within each of the samples.

If, on the other hand, the goal is to compare ability levels *between* the two samples, the context changes. For instance, we might ask whether university students score higher than the general population. Now what matters is not the signal-to-noise ratio within each group, but whether individual measurements carry the same degree of precision across groups. The difference in error score variance ($1$ in the student sample, $1.5$ in the general population) means that measurements in the general population sample are less precise: observed scores deviate more strongly from individuals' true ability levels than they do in the student sample. Reliability coefficients, being ratios, cannot detect this asymmetry. In both groups the true reliability coefficient is $\rho_{xx'} = .67$. This is the limitation that motivates moving from a relative to an absolute perspective on measurement. More broadly, when the research question concerns the precision of individual measurements, rather than how well a scale discriminates within a given sample, the absolute magnitude of error score variance is the more informative quantity.
...
Boot-Err has been developed in the context of the increasingly common situation in which a scale is administered across a large number of independent contexts, such as the Many Labs projects [e.g. @ManyLabs1], Registered Replication Reports [e.g. @RRRMazar] or research stemming from the psychological science accelerator [e.g. @psychologicalscienceacceleratorself-determinationtheorycollaboration2022]. The remainder of this text therefore provides a step-by-step tutorial for Boot-Err. We demonstrate how to estimate error score variance for each administration, approximate its sampling variability using Bootstrapping, and test and quantify heterogeneity in error score variance using random-effects meta-analysis, illustrated with data from a multi-lab replication project.

## Tutorial

Having compared three methods that each address measurement quality from a different angle, the remainder of this text focuses on Boot-Err. Because accessible tutorials already exist for Reliability Generalization Meta-Analysis and residual invariance testing, and as Boot-Err requires a rather technical implementation, the following section provides a hands-on guide to applying it in practice. The goal of this tutorial section is to compare error score variance of the same psychological scale across multiple independent administration contexts, and to test and quantify heterogeneity in random measurement error using Boot-Err.
...
The three methods discussed in this text each address this concern from a distinct angle. Reliability Generalization Meta-Analysis asks whetehr the signal-to-noise ratio of a scale holds across contexts. This is the right question, when the goal is to evaluate how well a scale discriminates within different groups. Residual invariance testing asks whether the measurement model holds equivalently. When a small, well-defined set of groups must be shown to be comparable, this is the right question to ask. Boot-Err asks both how much random measurement error varies across a potentially large number of independent administration contexts and whether that variation is statistically significant. This is the right question, when the research interest concerns the absolute magnitude and heterogeneity of measurement imprecision. Choosing among these methods should be driven by the research question, not by convention or availability.
