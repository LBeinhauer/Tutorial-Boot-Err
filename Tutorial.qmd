...
If the same instrument is administered in two different contexts, differences in random measurement error imply differences in measurement quality (ref.). However, the three methods discussed here do not address this in the same way. Instead, they reflect different perspectives on what it means for measurement quality to be equivalent, and they are therefore suited to different research questions.
...
Reliability Generalization Meta-Analysis extends this idea by formally testing whether observed differences in score reliability exceed what would be expected from sampling error alone [@vacha-haase1998]. Because coefficients derived from samples are always prone to sampling variability, one can not simply compare point estimates across administrations (ref.). Meta-analytic techniques model this sampling error derive an estimate of heterogeneity and test whether it is larger than 0 [@borenstein2009, @hunter2015]. Meta-analytic heterogeneity refers to variance, that exceeds the variation that sampling fluctuation alone would produce. If significant heterogeneity is detected, this indicates that reliability differs across administrations in a way, that is likely not attributable to chance (ref. actual rg meta-analysis).
...
\[before or after: section on why we compare two groups, even though in general, the methods are typically used to compare more than 2 groups. Other methods are available to compare 2 groups.\]
