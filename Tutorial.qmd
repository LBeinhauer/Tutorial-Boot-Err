...
Some things sound alike.
We explain what each individual thing does.
We identify use cases for each individual thing.
There are better tutorials and discussions out for RG-MA and MI.
What's missing is a tutorial on Boot-Err. That follows here.

Example use cases:

RG-MA
where is scale relatively more informative in one pop than the other
e.g. ability test - does it discriminate better among uni than general public
- students likely less variation among abilities than general public
essentially: signal to noise-ratio of interest. If rel largely equal, 
test can be used in both contexts to differentiate between levels of
ability

Residual Invariance Testing
cross-cultural CFA of e.g. personality dimension, needs to know whether
multi-factor scale meets statistical requirements for group comparison
(spec.: does measurement model hold equivalently)
Test is confirmatory: does model hold
likely less groups

Boot-Err
Same scale across larger number of labs / cultures, wants to know how
much measurement quality varies - e.g. how much does it contribute to
variability in effect sizes, is variance large enough to be relevant
...
The last level, the test for residual invariance is of largest interest, when discussing random measurement error in the data. Residual variance describes the variation in scores for a single item (or several), that can not be explained by the latent factor. In the terminology of classical test theory, this can be interpreted as error score variance (albeit there are some caveats, see ref XY). The test for residual invariance assesses whether these variances are identical across all groups. Thereby, measurement invariance testing allows fora formal test, in how far the measuring quality in terms of noise is equivalent across samples.

Returning to our exemplary case, in @eq-RGMA2 we observed that identical reliability coefficients might arise, even though measurement quality in terms of noise differed across samples. With some additional assumptions (unidimensionality, tau-equivalence, additive linear model - see reference), that we implicitly assume by averaging item responses to derive individual Extraversion scores, we can use the test for residual invariance to test for such differences in noise. If we observe a pattern like @eq-RGMA2, where error score variance was larger in the market research panel, the test for residual invariance (if power was adequate) should indicate that the test does not display residual invariance across samples. This means the test highlights that there are differences in measurement error (here referred to as residual variance). In a pattern like @eq-RGMA2, where reliability coefficients differed but noise was identical in market research panel and psychology student sample, the test for residual invariance should demonstrate just that (given adequate power) - there are no differences in random measurement error across administration sites.
