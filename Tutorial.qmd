...
Reliability Generalization Meta-Analysis extends this idea by formally testing whether observed differences in score reliability exceed what would be expected from sampling error alone [@vacha-haase1998]. Because coefficients derived from samples are always prone to sampling variability, one can not simply compare point estimates across administrations [@feldt1987, @cohen1994]. Meta-analytic techniques model this sampling error derive an estimate of heterogeneity and test whether it is larger than 0 [@borenstein2009, @hunter2015]. Meta-analytic heterogeneity refers to variance, that exceeds the variation that sampling fluctuation alone would produce. If significant heterogeneity is detected, this indicates that reliability differs across administrations in a way, that is likely not attributable to chance [@sánchez-meca2012, @rodriguez2006].
...
A second perspective is offered by measurement invariance testing, grounded in confirmatory factor analysis and typically implemented using structural equation modelling software such as lavaan or Mplus. Rather than summarising measurement quality as a single coefficient per sample, this framework models the relationship between latent factors and observed item responses and tests, whether this relationship holds equivalently across groups.
...
**Limitations** Importantly, the test for residual invariance generates sensible results only if the previous levels of invariance are established. If an earlier level could not be confirmed, for example, if scalar invariance does not hold, results from the test for residual invariance cannot be interpreted as evidence about differences in random measurement error, since the necessary preconditions for that interpretation are not met.
...
Accessible methodological guidance exists for Reliability Generalization Meta-Analysis (e.g. refs) and measurement invariance testing (e.g. refs).

Boot-Err has been developed in the context of the increasingly common situation in which a scale is administered across a large number of independent contexts. The reminder of this text therefore provides a step-by-step tutorial for Boot-Err. We demonstrate how to estimate error score variance for each administration, approximate its sampling variability using Bootstrapping, and test and quantify heterogeneity in error score variance using random-effects meta-analysis, illustrated with data from a multi-lab replication project.
